      ...
      <el-form>
        <el-form-item>
          <el-row :gutter="10" type="flex" align="middle">
            <el-col :span="5">
              <el-select v-model="reportType" placeholder="报告类型" size="small" style="width: 100%">
                <el-option
...
export default {
  ...
  data() {
    return {
      ...
      totalRecords: 10,
      tableCol: [
        { key: "orderNo", label: "订单编号", width: 160 },
            ...
            return (
              row.orderStatus === "0"? 
               <span  
                  ...
                  class="btnClass"
                  onClick={this.openOrderPayDialog.bind(this, 
                  row.orderNo,
                  row.orderName,
                  row.orderPrice,
                  row.actualAmount,
                  row.createDate,
                   row.payType)}> 去付款 </span>
                :row.orderStatus === "1" && !row.filePath?
                    <el-button type="text" disabled>报告生成中</el-button>
                :row.orderStatus === "1" && row.filePath?
                    <span class="btnClass" style="cursor:pointer" onClick={this.handleDownload.bind(this, row.filePath)}>报告下载</span>
                :row.orderStatus === "-1"?
                    <span class="btnClass" style="cursor:pointer" onClick={this.deleteOrder.bind(this, row.id)}>删除</span> : ' '
              // <button-dropdown
              //   data={[
              //     row.orderStatus === "0" && {
              //       text: "去付款",
