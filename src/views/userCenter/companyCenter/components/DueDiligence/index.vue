<template>
  <el-row class="credit-tab-content">
    <!-- 风险跟踪调查 -->
    <!-- <h4 class="flex" style="display:none">
      <span class="flex-primary">风险跟踪调查</span>
      <el-button size="mini" @click="$refs.ref_FollowUpInves.open({id})">新增风险</el-button>
    </h4>
    <el-row class="area"> -->
      <!-- <Comp0 ref="ref_Comp0"></Comp0> -->
      <!-- <TableWrapper
        ref="ref_tableFollow"
        :fetchDataFn="Api_trackingSurveyTable"
        :query="{id}"
        style="display:none"
        :columns="trackingSurveyCol"
      />
    </el-row> -->
    <!-- <FollowUpInves style="display:none" ref="ref_FollowUpInves" @getList="getTrackingSurveyTable"></FollowUpInves> -->

    <!-- 经营环境 -->
    <h4 class="flex">
      <span class="flex-primary">经营环境</span>
      <el-button size="mini" @click="visible1=true">{{ formData1.id ? '编辑' : '新增' }}</el-button>
    </h4>
    <el-row class="area">
      <Comp1 :data="formData1" is-view></Comp1>
    </el-row>
    <Dialog ref="drawer" :visible.sync="visible1" :onOk="handleOk1" title="新增经营环境" width="500px">
      <Comp1 ref="addForm1" :id="id" :companyName="companyName"></Comp1>
    </Dialog>

    <h4 class="flex">
      <span class="flex-primary">生产情况</span>
    </h4>
    <h5 class="flex">
      <span class="flex-primary">生产、经营场所</span>
      <el-button v-if="formData2.data1.length<3" size="mini" @click="addFormData2_1">新增</el-button>
    </h5>
    <el-row class="area">
      <el-table :data="formData2.data1">
        <template slot="empty">
        ...
        <el-table-column label="自有" prop="own"></el-table-column>
        <el-table-column label="按揭" prop="mortgage"></el-table-column>
        <el-table-column label="租赁" prop="lease"></el-table-column>
        <el-table-column label="操作">
          <template slot-scope="scope">
            <i class="el-icon-edit" style="cursor: pointer" @click="editFormData2_1(scope.row)"></i>
          </template>
        </el-table-column>
      </el-table>
    </el-row>
    <Dialog
      ref="drawer"
      :visible.sync="visible2_1"
      :onOk="handleOk2_1"
      :title="(modelStatus2_1 ? '新增' : '修改')+'生产、经营场所'"
      width="500px"
    >
      <Comp2_1
        ref="addForm2_1"
        :id="id"
        :modelStatus="modelStatus2_1"
        :sourceData="formData2.data1"
        :data="formData2Selected.data1"
      ></Comp2_1>
    </Dialog>
    <el-row class="area">
      <h5 class="flex">
        <span class="flex-primary">经营场所视察情况</span>
        <span>
          <el-button v-if="!formData2.data2.id" size="mini" @click="addFormData2_2">新增</el-button>
          <el-button v-else size="mini" @click="editFormData2_2">修改</el-button>
        </span>
      </h5>
      <Comp2_2 :id="id" :data="formData2.data2" is-view></Comp2_2>
    </el-row>
    <Dialog
      ref="drawer"
      :visible.sync="visible2_2"
      :onOk="handleOk2_2"
      :title="(modelStatus2_2 ? '新增' : '修改')+'经营场所视察情况'"
      width="500px"
    >
      <Comp2_2 ref="addForm2_2" :id="id" :modelStatus="modelStatus2_2" :data="formData2.data2"></Comp2_2>
    </Dialog>

    <h4 class="flex">
      <span class="flex-primary">主营业务及收入情况</span>
    </h4>
    <h5 class="flex">
      <span class="flex-primary">业务概况</span>
      <span>
        <el-button v-if="!formData3.data1.id" size="mini" @click="addFormData3_1">新增</el-button>
        <el-button v-else size="mini" @click="editFormData3_1">修改</el-button>
      </span>
    </h5>
    <el-row class="area">
      <Comp3_1 :id="id" :data="formData3.data1" is-view></Comp3_1>
    </el-row>
    <Dialog
      ref="drawer"
      :visible.sync="visible3_1"
      :onOk="handleOk3_1"
      :title="(modelStatus3_1 ? '新增' : '修改')+'业务概况'"
      width="500px"
    >
      <Comp3_1 ref="addForm3_1" :id="id" :modelStatus="modelStatus3_1" :data="formData3.data1"></Comp3_1>
    </Dialog>
    <h5 class="flex">
      <span class="flex-primary">收入构成</span>
      <span>
        <el-button size="mini" @click="addFormData3_2">新增</el-button>
      </span>
    </h5>
    <el-row class="area">
      <el-table :data="formData3.data2" :expand-row-keys="expandRowKeys" row-key="id">
        <template slot="empty">
          <img src="../../../../../assets/images/bg/no.jpg" alt="">
        </template>
        <el-table-column label="年度" prop="year"></el-table-column>
        <el-table-column label="营业收入合计(万元)" prop="totalIncome"></el-table-column>
        <el-table-column label="毛利润合计(万元)" prop="totalGrossProfit"></el-table-column>
        <el-table-column label="综合毛利率" prop="grossMargin"></el-table-column>
        <el-table-column label="操作">
          <template slot-scope="scope">
            <i class="el-icon-edit" style="cursor: pointer" @click="editFormData3_2(scope.row)"></i>
          </template>
        </el-table-column>
      </el-table>
    </el-row>
    <Dialog
      ref="drawer"
      :visible.sync="visible3_2"
      :onOk="handleOk3_2"
      :title="(modelStatus3_2 ? '新增' : '修改')+'收入构成'"
      width="500px"
    >
      <Comp3_2
        ref="addForm3_2"
        :id="id"
        :modelStatus="modelStatus3_2"
        :sourceData="formData3.data2"
        :data="formData3Selected.data2"
      ></Comp3_2>
    </Dialog>

    <h4 class="flex">
      <span class="flex-primary">供应商/客户</span>
    </h4>
    <h5 class="flex">
      <span class="flex-primary">前10大供应商</span>
      <span>
        <el-button size="mini" @click="addFormData4_1">新增</el-button>
      </span>
    </h5>
    <el-row class="area">
      <el-table :data="formData4.data1" row-key="id">
        <template slot="empty">
          <img src="../../../../../assets/images/bg/no.jpg" alt="">
        </template>
        <el-table-column label="序号" type="index" width="60"></el-table-column>
        <el-table-column label="供应商名称" prop="supplierName"></el-table-column>
        <el-table-column label="采购产品" prop="purchaseProduct"></el-table-column>
        <el-table-column label="采购金额(万元)" prop="purchaseAmount"></el-table-column>
        <el-table-column label="采购金额占比(%)" prop="proportionPurchase"></el-table-column>
        <el-table-column label="采购周期" prop="procurementCycle"></el-table-column>
        <el-table-column label="付款方式" prop="paymentMethod"></el-table-column>
        <el-table-column label="合作年限" prop="cooperationYears"></el-table-column>
        <el-table-column label="操作" width="100">
          <template slot-scope="scope">
            <i class="el-icon-edit" style="cursor: pointer" @click="editFormData4_1(scope.row)"></i>
            <i class="el-icon-delete" style="cursor: pointer" @click="delFormData4_1(scope.row)"></i>
          </template>
        </el-table-column>
      </el-table>
    </el-row>
    <Dialog
      ref="drawer"
      :visible.sync="visible4_1"
      :onOk="handleOk4_1"
      :title="(modelStatus4_1 ? '新增' : '修改')+'供应商'"
      width="500px"
    >
      <Comp4_1 ref="addForm4_1" :id="id" :modelStatus="modelStatus4_1" :data="formData4Selected.data1"></Comp4_1>
    </Dialog>
    <h5 class="flex">
      <span class="flex-primary">前10大客户</span>
      <span>
        <el-button size="mini" @click="addFormData4_2">新增</el-button>
      </span>
    </h5>
    <el-row class="area">
      <el-table :data="formData4.data2" row-key="id">
        <template slot="empty">
          <img src="../../../../../assets/images/bg/no.jpg" alt="">
        </template>
        <el-table-column label="序号" type="index" width="60"></el-table-column>
        <el-table-column label="客户名称" prop="customerName"></el-table-column>
        <el-table-column label="销售产品" prop="promotingProducts"></el-table-column>
        <el-table-column label="销售金额(万元)" prop="salesAmount"></el-table-column>
        <el-table-column label="销售金额占比(%)" prop="proportionSales"></el-table-column>
        <el-table-column label="回款周期" prop="returnPeriod"></el-table-column>
        <el-table-column label="付款方式" prop="paymentMethod"></el-table-column>
        <el-table-column label="合作年限" prop="cooperationYears"></el-table-column>
        <el-table-column label="操作" prop="lease" width="100">
          <template slot-scope="scope">
            <i class="el-icon-edit" style="cursor: pointer" @click="editFormData4_2(scope.row)"></i>
            <i class="el-icon-delete" style="cursor: pointer" @click="delFormData4_2(scope.row)"></i>
          </template>
        </el-table-column>
      </el-table>
    </el-row>
    <Dialog
      ref="drawer"
      :visible.sync="visible4_2"
      :onOk="handleOk4_2"
      :title="(modelStatus4_2 ? '新增' : '修改')+'客户'"
      width="500px"
    >
      <Comp4_2 ref="addForm4_2" :id="id" :modelStatus="modelStatus4_2" :data="formData4Selected.data2"></Comp4_2>
    </Dialog>
