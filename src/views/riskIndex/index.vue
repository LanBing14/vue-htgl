<template>
  <el-container style="background-color: #F6F9FA">
    <el-header>
      <FxbHeader></FxbHeader>
    </el-header>
    <el-main class="content">
      <div style="background-color: #fff; padding: 17px 20px; margin-bottom: 15px">
        ...
        <div class="page-content">
          <el-form ref="searchForm" :model="form" :inline="true">
            ...
            <el-form-item prop="industry">
              <el-select v-model="form.industry" size="small" placeholder="全部行业">
                <el-option
            ...
            <!-- <el-form-item prop="area">
              <el-select v-model="form.area" size="small" placeholder="按省级区域">
                <el-option label="item.label" value="item.value"></el-option>
              </el-select>
            </el-form-item>-->
            <el-form-item>
              <el-button type="primary" size="small" @click="fetchData" icon="el-icon-search">查询</el-button>
              <el-button size="small" @click="resetData" icon="el-icon-refresh">重置</el-button>
            </el-form-item>
          </el-form>
        </div>
      </div>
      <el-row :gutter="15" style="margin-bottom: 15px">
        <el-col :span="16" style="height: 560px">
          <div style="background-color: #fff; height: 100%; padding: 20px" v-loading="loading_map">
            <div class="risk-title" style="margin-bottom: 0">区域排名</div>
            <div class="risk-content" style="height: calc(100% - 18px)" id="echart_map"></div>
          </div>
        </el-col>
        <el-col :span="8" style="height: 560px">
          <div
            style="background-color: #fff; height: 100%; padding: 20px; overflow-y: hidden"
            v-loading="loading_listA"
          >
            <div class="risk-title">区域排名</div>
<!--            <el-scrollbar style="height: calc(100% - 40px)">-->
              <div class="risk-content">
                <el-table :data="listDataA" :height="480" stripe :default-sort="{prop: 'indexScorce', order: 'descending'}">
                  ...
                  >
                    <template slot-scope="scope">
                      <span class="canClick">{{ !area?scope.row.indexProvince:scope.row.indexCity }}</span>
                    </template>
                  </el-table-column>
                  <el-table-column label="风险指数" prop="indexScorce" width="130" sortable align="center">
                    <template slot-scope="scope">{{scope.row.indexScorce}}</template>
                  </el-table-column>
                </el-table>
              </div>
<!--            </el-scrollbar>-->
      ...
      <el-row :gutter="15">
        <el-col :span="16" style="height: 320px">
          <div style="background-color: #fff; height: 100%; padding: 20px" v-loading="loading_chart">
            <div class="risk-title">趋势分析</div>
            <div class="risk-content" id="echart_index"></div>
          </div>
        </el-col>
        <el-col :span="8" style="height: 320px">
          <div
            style="background-color: #fff; height: 100%; padding: 20px; overflow-y: hidden"
            v-loading="loading_listB"
          >
            <div class="risk-title">
              ...
              <span
                  class="subtitle-item"
                  :class="{'text-blue':industryType===1}"
                  @click="industryType=1;getListB()"
              >按指数</span>
                  <!--<span-->
                  <!--class="subtitle-item"-->
                  <!--:class="{'text-blue':industryType===2}"-->
                  <!--@click="industryType=2;getListBC()"-->
                  <!--&gt;按趋势</span>-->
              </div>
            </div>
<!--            <el-scrollbar style="height: calc(100% - 40px)">-->
              <div class="risk-content">
                <el-table :data="listDataB" :height="240" stripe :default-sort="{prop: 'indexScorce', order: 'descending'}">
                  <el-table-column label="排名" width="80" align="center">
                    <template slot-scope="scope">
                      <span
                        :class="{['ph-color'+ (scope.$index + 1)]: scope.$index<3}"
                  ...
                  >
                    <template slot-scope="scope">
                      <span
                        class="canClick"
                      >{{ industryType===1?scope.row.indexIndustry:scope.row.indexTime }}</span>
                    </template>
                  </el-table-column>
                  <el-table-column label="风险指数" prop="indexScorce" sortable width="130" align="center">
                    <template slot-scope="scope">{{scope.row.indexScorce}}</template>
                  </el-table-column>
                </el-table>
              </div>
...
import {
  Api_analysisResultsMap,
  Api_analysisResultsArea,
  Api_analysisResultsTrend,
  Api_analysisResultsWork,
  Api_analysisResultsWorkTrend,
  Api_analysisResultsMonth,
} from "@/api/creditManagement/index.js";
export default {
  name: "riskIndex",
  components: { FxbHeader },
  data() {
    return {
      form: { month: null, industry: null, area: null },
      industryType: 1,
      loading_map: false,
      loading_chart: false,
      loading_listA: false,
  ...
  methods: {
    fetchData() {
      this.myChartData((res) => {
         // // item.itemStyle= { normal: { areaColor:'#AADDFF'}, emphasis: { areaColor: '#c5e6fd'}};
         // let data = res.filter((a,i)=>{
         //      if(i===0){
         //          a.selected = true
         //      }
         //      a.itemStyle = {
         //         normal: { areaColor: '#000' },
         //         emphasis: { areaColor: '#333444'}
         //      };
         //      console.log(a);
         //      return a
         //  });
    ...
    myChartData(cb) {
      ...
      Api_analysisResultsMap(this.form.month, { work: this.form.industry })
        .then((res) => {
          this.loading_map = false;
          cb &&
            cb(
              res.map((item) => ({
                name: item.indexProvince,
                value: item.indexSource,
                tb: item.tb,
                hb: item.hb,
              }))
            );
    ...
    myChart(res) {
      const _that = this;
      const myChart = echarts3.init(document.getElementById("echart_map"));
      var mapName = "china";
      var data = res;
      var geoCoordMap = {};
      /*获取地图数据*/
      // myChart.showLoading();
      var mapFeatures = echarts3.getMap(mapName).geoJson.features;
      // myChart.hideLoading();
      ...
      var option = {
        tooltip: {
          ...
          formatter: function (params) {
            // console.log(params)
            if (!params.name) {
              return `<div style="background-color:rgba($color:#333,$alpha:0.3);color:#fff;padding:8px 10px">暂无数据</div>`;
            }
            return `<div style="background-color:rgba($color:#333,$alpha:0.3);width:200px;color:#fff;padding:8px 10px">
                <div style="display:flex">
                  <div style="flex:1">区域名称</div>
                  <div style="flex:1;text-align:right">${params.name}</div>
                </div>
                <div style="display:flex">
                  <div style="flex:1">风险指数</div>
                  <div style="flex:1;text-align:right">${
                    params.componentSubType === "effectScatter" ? params.value[2] : params.value
                  }</div>
                </div>
                <div style="display:flex">
                  <div style="flex:1">同比</div>
                  <div style="flex:1;text-align:right">${params.data.tb}</div>
                </div>
                <div style="display:flex">
                  <div style="flex:1">环比</div>
                  <div style="flex:1;text-align:right">${params.data.hb}</div>
                </div>
              </div>`;
            // setTimeout(function () {
        ...
        geo: {
          ...
          roam: false,
          itemStyle: {
            normal: { areaColor: "#409eff", borderColor: "#f8f9fa" },
            emphasis: { areaColor: "#1C86EE" },
          },
          regions: []
        },
        series: [
      ...
      myChart.on("click", function (params) {
          ...
          if (params.componentSubType === "effectScatter" || params.componentSubType === "map") {
          _that.area = params.name;
          _that.getListA();
          _that.indexChartData((res) => {
            _that.indexChart();
          });
          _that.industryType === 1 && _that.getListB();
          _that.industryType === 2 && _that.getListBC();
        }
      });
    },
    indexChartData(cb) {
      this.loading_chart = true;
      Api_analysisResultsTrend(this.form.month, { area: this.area, work: this.form.industry })
        .then((res) => {
    ...
    indexChart(res) {
      const indexChart = echarts3.init(document.getElementById("echart_index"));
      const option = {
        tooltip: {
          trigger: "axis",
          axisPointer: {
        ...
        series: [
          {
            ...
            type: "bar",
            barWidth: "13",
            data: res.map((i) => i.indexScorce),
            itemStyle: {
              normal: {
                barBorderRadius: [3, 3, "", ""],
                color: new echarts3.graphic.LinearGradient(0, 0, 0, 1, [
                  { offset: 0, color: "#67A8F8" },
                  { offset: 1, color: "#76B1F9" },
                ]),
              },
            },
          },
    ...
    getListA() {
      this.loading_listA = true;
      Api_analysisResultsArea(this.form.month, { area: this.area || "" })
        .then((res) => {
          this.loading_listA = false;
          this.listDataA = res;
        })
        .catch((res) => {
          this.loading_listA = false;
    ...
    getListB() {
      this.loading_listB = true;
      Api_analysisResultsWork(this.form.month, { area: this.area })
        .then((res) => {
          this.loading_listB = false;
          this.listDataB = res;
        })
        .catch((res) => {
          this.loading_listB = false;
    ...
    getListBC() {
      this.loading_listB = true;
      Api_analysisResultsWorkTrend(this.form.month, { area: this.area, work: this.form.industry })
        .then((res) => {
          this.loading_listB = false;
          this.listDataB = res;
        })
