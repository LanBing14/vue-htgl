<template>
  <div>
    <h4 class="flex">
     ...
     <el-button v-if="!isFxb" size="mini" @click="open">{{ companyInfo.id ? '编辑': '新增' }}</el-button>
    </h4>
    ...
    <Dialog
      ...
      @close="onClose"
    >
      <el-form ref="form" :model="formData" :rules="formRules" label-width="11em" label-position="right">
  ...
  methods: {
    ...
    handleSure() {
      this.$refs.form.validate(valid => {
        ...
        const promiseRes = !id
          ? Api_CustomerArchivesCompanyInfoAdd(this.formData.companyId, postData)
