<template>
  <div>
    <h4 class="flex">
      <span class="flex-primary">清算信息</span>
      <el-button size="mini" @click="openModal()" v-if="!isFxb">新增</el-button>
    </h4>
    <el-row>
      <TableWrapper
        ref="table"
        :fetchDataFn="Api_CustomerArchivesLiquidationInfo"
        :query="listQuery"
        :columns="tableCol1"
      />
    </el-row>
    ...
    >
      <el-form ref="form" :model="formData" :rules="formRules" label-width="8em" label-position="right">
        <el-form-item label="清算组负责人" prop="leader">
          <el-input v-model="formData.leader"></el-input>
        </el-form-item>
        <el-form-item label="清算组成员" prop="member">
...
import {
  Api_CustomerArchivesLiquidationInfo,
  Api_CustomerArchivesLiquidationAdd,
  Api_CustomerArchivesLiquidationUpdate,
  Api_CustomerArchivesLiquidationDelete
  ...
  methods: {
    ...
    handleSure() {
      this.$refs.form.validate(valid => {
        ...
        const promiseRes = !id
          ? Api_CustomerArchivesLiquidationAdd(this.formData.companyId, postData)
          : Api_CustomerArchivesLiquidationUpdate(id, this.formData.companyId, {
              ...postData,
    ...
    handleDelete(row) {
      this.$confirm("确认删除？", "提示", {
        ...
        const loading = this.$openLoading();
        Api_CustomerArchivesLiquidationDelete(row.id, this.formData.companyId)
          .then(res => {
            loading.close();
            this.getList();
            this.$message.success(res);
