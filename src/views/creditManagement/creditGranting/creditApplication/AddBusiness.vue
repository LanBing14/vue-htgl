<template>
  <Dialog ref="drawer" class="size-btn-change" title="添加企业" :visible.sync="visible" :btns="drawerBtns" width="500px" @close="onClose">
...
export default {
  ...
  methods: {
    open(_a, _s) {
      ...
      Api_GetGroups().then(res => {
        this.groupsOptions = res;
      });

      if (_s) {
        this.groupIdsOptions = _a.options.filter(i => i.group_name !== "未分组");
        ...
        Api_GetCustomerGroup().then(res => {
          this.groupIdsOptions = res.filter(i => i.group_name !== "未分组");
        });
      }
    },
    checkName() {
