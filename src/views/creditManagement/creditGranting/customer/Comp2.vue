    ...
    >
        ...
        >
            ...
            <!--<el-form-item label="单笔限额" prop="singleQuota">
                <el-input v-model="formData.singleQuota" placeholder="请填写单笔限额">
                    <template slot="append">元</template>
                </el-input>
            </el-form-item>-->
            ...
            <el-form-item label="允许逾期天数" prop="overdueDays">
                <el-input v-model.number="formData.overdueDays" placeholder="请填写允许逾期天数">
                    <template slot="append">天</template>
                </el-input>
            </el-form-item>
...
<script>
    export default {
        ...
        methods: {
            open(data, index) {
                this.formData.modelGradeHisId = data.modelGradeHisId;
            ...
            handleSure() {
                this.$refs.form.validate(valid => {
                    if (valid) {
                        this.formData.creditQuota = this.$turnMoney(this.formData.creditQuota, true);
                        // this.formData.singleQuota = this.$turnMoney(this.formData.singleQuota, true);
                        this.formData.overdueQuota = this.$turnMoney(this.formData.overdueQuota, true);
                        this.$emit('ok', Object.assign({}, this.formData), this.index);
                        this.visible = false;
                    }
                });
            },
