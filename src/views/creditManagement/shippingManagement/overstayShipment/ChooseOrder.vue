    ...
    >
      <el-form-item>
        <el-input
          ...
          placeholder="请输入订单编码/名称"
          class="filter-item change-input-size"
          @blur="clearAll"
...
import { Api_specialSendContract } from "@/api/creditManagement/shippingManagement/overstayShipment.js";
...
export default {
  ...
  watch: {
    cID(_v) {
      if (_v) {
        this.loading = true;
        Api_specialSendContract({ customerId: this.cID })
          .then(res => {
            this.loading = false;
            if (!res && !res.length) {
              this.$emit("nonContract");
              return false;
            }
    ...
    handleFilter() {
      this.filteredTableData = this.tableData.filter(data => {
        return (
          ...
          data.contractName.toLowerCase().includes(this.listQuery.contractName.toLowerCase())
        );
      });
    },
    handleReset() {
