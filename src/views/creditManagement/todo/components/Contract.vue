                ...
                <el-row>
                    <el-col :span="8">
                        <el-form-item label="客户名称">
                            <span class="canClick"  @click="$gotoCustomerDetail(form.customerId)">
                            {{ form.customerName }}
                            </span>
                        </el-form-item>
                    </el-col>
                    ...
                    <el-col :span="8">
                        <el-form-item
                            label="合同状态"
                        >{{ form.state==="0"?"合同拟定":form.state==="1"?"合同审核":form.state==="2"?"合同签约":form.state==="3"?"合同执行":form.state==="4"?"履约完成":form.state==="5"?"合同取消":"合同作废" }}</el-form-item>
                    </el-col>
                    <el-col :span="24">
                        <el-divider class="detail-divider"></el-divider>
                    </el-col>
                    ...
                    <el-tab-pane label="合同申请信息">
                        ...
                        <el-row>
                            <el-col :span="8">
                                ...
                                >
                                    <!-- <span v-if="fileList&&fileList.length" class="canClick"
                                          @click="viewPdfs">点击查看</span>
                                    <span v-else>-</span> -->
                        ...
                        <el-row>
                            <el-col :span="8">
                                <el-form-item label="订单发货控制">
                                    <el-switch v-model="form.controller" disabled></el-switch>
                                </el-form-item>
                            </el-col>
                        </el-row>
                        <el-divider class="detail-divider"></el-divider>
...
import { Api_AgencyMattersGetContractInfo } from "@/api/creditManagement/todo/index.js";
...
export default {
        ...
        return {
            ...
            paymentPlanCol: [
                ...
                },
                {
                    ...
                    key: "planAmount",
                    render: (h, data, row) => {
                        return ((Number(data) * 100) / Number(this.form.contractAvailable)).toFixed(2) + "%";
                    }
                },
                {
                    label: "计划付款日期",
    ...
    mounted() {
        console.log(this.row, this.modelStatus);

        const id = this.row.business_key;
        this.cID = id;
        this.getDetail(id);
    },
    methods: {
        getDetail(id) {
            ...
            Api_AgencyMattersGetContractInfo(id)
                .then(res => {
                    loading.close();
                    this.form = res;
                    if (res.additionalUploadFiles && res.additionalUploadFiles.length) {
                        this.fileList = res.additionalUploadFiles.length
                            ? res.additionalUploadFiles.map(item => {
                                  return { url: item.url, name: item.originalName, domain: item.domain };
                              })
                            : [];
                    } else {
