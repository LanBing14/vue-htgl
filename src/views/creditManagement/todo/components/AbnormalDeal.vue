    ...
    <div class="page-content">
      <el-form ref="form" v-model="displayInfo" label-width="110px">
        ...
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="签收人">{{ displayInfo.arrivalInfo.name }}</el-form-item>
          </el-col>
          <el-col :span="8">
        ...
        <el-form-item label="异常处理附件">
          ...
          <!-- <div v-if="details.files&&details.files.length" class="demo-image__preview">
            <el-image
              ...
              :key="index"
              style="width: 100px; height: 100px; margin-right: 6px"
              :src="baseFileUrl+item.url"
              :preview-src-list="details.files.map(i=>baseFileUrl+i.url)"
            ></el-image>
          </div>
          <span v-else>无</span> -->
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      baseFileUrl: process.env.VUE_APP_FILE_API,
  ...
  mounted() {
    ...
    Api_getAbnormalDetail(this.row.business_key)
      .then((res2) => {
        ...
        Api_ArrivalDetail(res2.shipmentId)
          .then((res) => {
