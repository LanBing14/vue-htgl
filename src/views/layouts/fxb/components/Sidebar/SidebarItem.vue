...
<!--common-->
  <div v-if="!item.hidden" class="menu-wrapper">
    <template v-if="!item.children || !item.children.length || item.hiddenChildren">
      <!-- <app-link
        :to="resolvePath(item.hiddenChildren?'':(item.alias||item.path))"
        :new-window="item.window"
      > -->
      <!-- 这里为了点击同路由页面刷新修改 -->
    ...
    <el-submenu v-else ref="submenu" :index="resolvePath(item.alias||item.path)">
      ...
      <template v-for="child in item.children" v-if="!child.hidden">
        ...
        > -->
        <!-- 这里为了点击同路由页面刷新修改 -->
        <a :key="child.name" @click="linkClick(resolvePath(child.alias||child.path))" v-else >
